	...
	<div class="i_footer">
		<div class="footer_content clearfix">
			<div class="footer_msg" v-for="(item, index) in list" :key="index">
				<ul>
					...
					<li class="m_row"></li>
					<li v-for="(that, num) in item.children" :key="num"><a :href="'/terrace/'+that.id">{{that.name}}</a>
					</li>
				</ul>
			</div>
			<div class="m_right">
		...
		<div class="friend_link">
			<ul class="clearfix">
				...
				<li v-for="(item, index) in friendList" :key="index"><a :href="item.linkUrl"
						target="_blank">{{item.linkName}}</a></li>
			</ul>
		</div>
		<div class="copyright">
		...
		<div class="icp_num">
			...
			<a :href="'http://www.beian.gov.cn/portal/index'" target="_blank" class="lingke_link"><img
					src="@/assets/image/prn_icon.png" class="prn_icon" alt="">&nbsp;{{service.prn}}</a>
		</div>
	</div>
</template>
<script>
