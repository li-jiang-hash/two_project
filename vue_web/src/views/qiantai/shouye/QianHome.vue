            ...
            <div class="i_zone" >
                <div class="zone_header"  >
                    <div class="big_text">
                        ...
                        <router-link :to="{name: 'list', params: {id: item.id,zoneName: item.zoneName,zoneDesc: item.zoneDesc}}" class="fr small_text link_text">更多店铺 ></router-link>
                    </div>
                </div>
                <div class="zone_body">
                    <ul class="clearfix business_list">
                        <li v-for="(that, int) in business" :key="int" v-if="that.zoneId==item.id">
                            <router-link  :to="{name: 'BusinessXinX', params: {id: that.bid}}">
                                <div class="img_box" >
                                    <img :src="that.businessInfo.bicon" alt="">
                                </div>
                                <!-- 店铺名 -->
                                <p style="font-size: 18px;color:red;font-family: 黑体">{{ that.businessInfo.sname }}</p>

                                <!-- 成交信息 -->
                                <p class="productStatus">
                                    <span v-if="that.businessInfo.sales!=null">总成交<span style="font-size: 14px;">{{that.businessInfo.sales}}</span>单</span>
                                    <span v-if="that.businessInfo.sales==null" style="font-size: 14px;color: #8c939d">暂未出单</span>
                                </p>

                                <div class="price_box" v-if="that.businessInfo.minprice!=null">￥{{Number(that.businessInfo.minprice).toFixed(2)}}起</div>
                                <div class="price_box" v-if="that.businessInfo.minprice==null">￥0.00起</div>
                            </router-link>
                        </li>
                    </ul>
                    <ul class="test_list clearfix">
                        <li :class="{test_option: true, right_0: (num % 2 == 1)}" v-for="(resource, num) in item.zoneCourseLibList" :key="resource.id">
                            <router-link target="_blank" :to="{name: 'libraryDetail', params: {id: resource.id}}"><i class="iconfont">&#xe6be;</i>{{resource.courseName}}</router-link>
                        </li>
                    </ul>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        created() {
            this.centerDialogVisible = false;
            var that=this;
            this.$http.get("home/zone/prefecture").then(function (resp) {
                that.zoneData=resp.data.data;
                //console.log(that.zoneData)
            })
            this.$http.get("home/zoneBusiness/selectZoneBusiness").then(resp => {
