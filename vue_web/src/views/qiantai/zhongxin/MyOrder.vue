            ...
            <div class="order_content" v-for="(item, index) in pageObj.order" :key="index">
                ...
                <div class="order_body clearfix">
                    <div class="body_left clearfix" v-for="(items,index) in item.goodsList" :key="index">
                        ...
                        <router-link v-if="item.commentstatus===0 && item.state===0" :to="{name: 'DetailId',params:{id:items.goodid}}">
    ...
    export default {
        ...
        methods: {
            ...
            initOrderList() {
                this.notdata = false;
                var that = this;
                this.$http.get(`/order/order/findOrder/${this.pageObj.pageCurrent}/${this.pageObj.pageSize}` + "?zhuangtai=" + this.zhuangtai).then(function (resp) {
