	...
	<div>
						...
						<el-table ref="cartTable" :data="cartList" v-loading="loading" size="mini"
							...
							<el-table-column label="商品图片" width="180" header-align="center" align="center">
								<template slot-scope="scope">
									<el-image :src="scope.row.img" style="width: 192px; height: 108px">
										<div slot="placeholder" class="image-slot">
											Loading<span class="dot">...</span>
										</div>
										<div slot="error" class="image-slot">
											<el-image style="width: 192px; height: 108px"
							...
							<el-table-column header-align="center" align="center" label="商品名称" prop="gname">
							</el-table-column>
							<el-table-column header-align="center" align="center" label="单价" prop="price">
							</el-table-column>
							<el-table-column header-align="center" align="center" label="数量" width="220">
								<template slot-scope="scope">
									库存剩余:{{scope.row.residue}}
									<el-input-number setp="1" :min="1" :max="scope.row.residue" v-model="scope.row.num"
										plain v-on:change="calTotalPrice()"></el-input-number>
								</template>
							</el-table-column>
							...
							<el-table-column header-align="center" align="center" label="操作" fixed="right">
								<template slot-scope="scope">
									<el-button size="medium" type="danger" v-on:click="delGood(scope.row.id)">删除
									</el-button>
								</template>
							</el-table-column>
						</el-table>
					...
					<div class="cart-filter-bar-bottom">
						<div class="bar-bottom-left">
							<span class="all-check">
								<div>
									<el-checkbox class="all-check-checkbox" v-model="isAllChecked"
										v-on:change="checkAll">全选</el-checkbox>
								</div>
							</span>
		...
		<el-dialog title="收银台" :visible.sync="dialogFormVisible" width="600px" @close="flush">
			<div data-v-1ee14d65="" style="margin: auto">
				<div
					style="float: right;margin-right: 40px;color: red;font-size: 20px;font-family: 幼圆;margin-top: -37px;">
					总价:￥{{cartTotalPrice}}
				</div>
				<div data-v-1ee14d65="" class="remark" style="margin-top: 23px;padding-left: 75px">
					<div style="margin-bottom: 20px;margin-left: -73px;border-radius: 10px;width: 535px;border: 1px solid darkgray;box-shadow: 10px 10px 5px rgba(0,0,0,0.9)"
						v-for="item in orderInfo">
						<div>
							<div style="float: left;margin-top: 10px;margin-left: 10px" class="block">
								<el-avatar shape="square" :size="35" :src="item[0].bicon"></el-avatar>
							</div>
						</div>
						...
						<div style="margin-top: 20px" v-for="items in item">
							...
							<div
								style="margin-left: 418px;margin-top: -40px;margin-bottom: 20px;border: 2px solid grey;border-radius: 10px;width:42ppx;height: 26px;padding-top: 5px;padding-left: 4px;">
								×{{items.num}}
							</div>
					...
					<el-form ref="form" :model="payForm" style="margin-left: -69px">
						...
						<el-select size="small" v-model="payForm.addrid" placeholder="请选择收货地址" @change="aaaa">
							<el-option v-for="item in addrForm" :label="item.addr" :value="item.id">
							</el-option>
						</el-select>
						<el-link type="primary" style="font-family: 幼圆;font-size: 15px;margin-left: 20px"
							href="http://localhost:8080/geRenCenter">点我添加新的地址
						</el-link>
					</el-form>
				</div>
				<el-button type="primary" style="margin-left: 210px;margin-top: 25px;" @click="choosePay">立即购买
				</el-button>
		...
		<el-dialog title="请选择支付类型" :visible.sync="payDialogVisible" width="50" @close="aboutpay">
			...
			<div style="height: 520px;margin-top: 20px;margin-left: 100px">
				...
				<span id="weixin" style="display: inline-block;margin-left: 60px;border: 2px solid lightgrey">
				</span><br>
			...
			choosePay() {
				if (this.payForm.addrid != null && this.payForm.addrid !== "") {
					...
					for (var i = 0; i < this.selectionData.length; i++) {
						delete this.selectionData[i].img
						delete this.selectionData[i].checked
						this.selectionData[i].cartid = this.selectionData[i].id
						delete this.selectionData[i].id

						this.selectionData[i].goodsid = this.selectionData[i].goods_id
						delete this.selectionData[i].goods_id
						this.selectionData[i].uid = sessionStorage.getItem("userId")

						this.selectionData[i].addrid = this.payForm.addrid
						this.selectionData[i].totalmoney = this.selectionData[i].price * this.selectionData[i].num
						this.selectionData[i].state = 1
					}

					var goodsids = [];
					var nums = []
					for (var i = 0; i < this.selectionData.length; i++) {
						goodsids[i] = this.selectionData[i].goodsid
						nums[i] = this.selectionData[i].num
					}
					//减去商品数量
					...
					res => {
						if (res.data.data) {
							//生成订单
							this.$http.post("sys-order/o-order/settlement", this.selectionData).then(res => {
								if (res.data.data) {
									...
									var ids = [];
									for (var i = 0; i < this.selectionData.length; i++) {
										ids[i] = this.selectionData[i].cartid
									}
									//删除购物车
									this.$http.get("sys-order/o-cart/deleteCart?id=" + ids).then(res => {
										if (res.data.data) {
											this.getList();
										}
									})
