		...
		<el-card class="box-card">
			...
			<div>
				<el-table v-loading="ctrl.loading" size="medium" :data="tableData" stripe border style="width: 100%">
					<el-table-column prop="cardfront" label="商品图片" width="122" fixed header-align="center"
						align="center">
						...
						<template slot-scope="scope">
							<ul class="list-item-actions">
								<li>
									...
									<el-button v-if="scope.row.status === 1 && scope.row.state !== 0" disabled
										type="success" size="mini">上架</el-button>
									<el-button v-if="scope.row.status === 0" type="warning"
										@click="changeStatus(scope.row)" size="mini">下架</el-button>
								</li>
							</ul>
						</template>
					</el-table-column>
			...
			<el-dialog title="商品信息" :visible.sync="addDialogFormVisible">
				<el-form :model="addForm" style="width: 400px;">
					...
					<el-form-item label="保质期" label-width="80px">
						<el-date-picker value-format="yyyy-MM-dd hh:mm:ss" v-model="addForm.expirationtime"
			...
			<el-dialog title="商品信息" :visible.sync="updateDialogFormVisible">
				<el-form :model="updateForm" style="width: 400px;">
					...
					<el-form-item label="保质期" label-width="80px">
						<el-date-picker value-format="yyyy-MM-dd hh:mm:ss" v-model="updateForm.expirationtime"
							type="datetime" placeholder="选择日期时间">
						</el-date-picker>
					</el-form-item>
					<el-form-item label="商品价格" label-width="80px">
...
<script>
	...
	export default {
		...
		methods: {
			getSort() {
				this.$http.post("syssystem/g-sort/getSort").then(res => {
					console.log(res.data.data);
					if (res.data.code === 2000) {
						this.SortForm = res.data.data;
			...
			deleteGoods(id) {
				this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
					...
					this.$http.post("syssystem/g-goods/deleteGoodsById/" + id).then(res => {
						if (res.data.code === 2000) {
							this.$message({
								type: 'success',
								message: '删除成功!'
