...
<!--                </el-table-column>-->
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button
                                plain
                                @click="open(scope.row.roleName,scope.row.scontend)">
                            查看公告
                        </el-button>
                    </template>
                </el-table-column>
...
<script>
    export default {
        ...
        methods: {
            ...
            initTable() {
                const that = this;
                that.$http.post("/emp/message/getMessage?currentPage=" + this.currentPage + "&pageSize=" + this.pageSize + "&startTime=" + this.startTime + "&endTime=" + this.endTime).then(function (response) {
            ...
            del(row, id) {
                this.$confirm(`确定删除该${row.stype}吗？, 是否继续?`, '提示', {
                    ...
                    this.$http.post(`/emp/message/deleteMessageById/${id}`).then((resp) => {
            ...
            insertFormSubmit() {
                ...
                this.$refs.insertForm.validate(valid => {
                    if (valid) {
                        that.$http.post(`/emp/message/sendMessage`, this.insertFormDate).then(function (resp) {
                            if (resp.data.code === 2000) {
                                that.initTable();
