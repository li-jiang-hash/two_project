        ...
        <div>
            <el-form :inline="true" :model="map" size="mini">
                <el-form-item label="储藏方式:">
                    <el-select v-model="map.depositid" filterable placeholder="请选择商品类型" style="width: 130px">
                        <el-option
                                v-for="item in sorts"
...
/*    import Add from './add'
    ...
    export default {
        ...
        methods: {
            ...
            submitCheckForm(){
                var that = this
                this.$http.post(`/commodity/deposit/updateStatus`,this.checkformData).then(function (resp) {
            ...
            searchSubject(){
                var that =this
                this.$http.post(`/commodity/deposit/findDepositByCondition/${this.page.pageCurrent}/${this.page.pageSize}`,this.map).then(function (resp) {
                    if (resp.data.code===2000){
            ...
            handleDelete(id) {
                ...
                this.$confirm(`确定要删除吗?`, {
                    ...
                    this.$http.post("/commodity/deposit/deleteDeposit?id="+id).then(function (resp) {
                    console.log(resp)
                    if (resp.data.code===2000){
                        that.$message.success(resp.data.msg);
                    }
