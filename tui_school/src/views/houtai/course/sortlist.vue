        ...
        <div>
            <el-form :inline="true" :model="map" size="mini">
                <el-form-item label="商品类型:">
                    <el-select v-model="map.sortname" filterable placeholder="请选择商品类型" style="width: 130px">
                        <el-option
                                v-for="item in sorts"
                                :label="item.sortname"
                                :value="item.sortname">
...
import qs from 'qs'
    export default {
        ...
        methods: {
            ...
            searchSubject(){
                var that =this
                this.$http.post(`/syssystem/g-sort/findSorts?currentPage=${this.page.currentPage}&pageSize=${this.page.pageSize}`,qs.stringify(this.map)).then(function (resp) {
         ...
         // //查询商品类型
            findAllBrands(){
                    this.$http.get(`/syssystem/g-sort/findSort`).then(res=>{
            ...
            submitCheckForm(){
                var that = this
                this.$http.post(`/syssystem/g-sort/shenhe`,this.checkformData).then(function (resp) {
                    if (resp.data.code===2000){
            ...
            handleDelete(id) {
                ...
                this.$confirm(`确定要删除吗?`, {
                    ...
                    this.$http.post(`/syssystem/g-sort/${id}`).then(function (resp) {                   
                    if (resp.data.code===2000){
                        that.$message.success(resp.data.msg);
                    }
                    that.searchSubject()
