    ...
    <el-card class="box-card">
      ...
      >
        <el-form-item label="链接名称">
          <el-input
            v-model="searchformData.linkName"
            placeholder="请输入链接名称"
          ></el-input>
        </el-form-item>
        <el-form-item label="状态：">
          <el-select
            v-model="searchformData.isDisable"
            class="auto-width"
            clearable
            filterable
            placeholder="状态"
        ...
        >
          ...
          >
            <template slot-scope="scope"
              ><el-link
                type="primary"
                @click="jumpUrl(scope.row.linkUrl)"
                target="_blank"
                >{{ scope.row.linkName }}</el-link
              >
              【{{ scope.row.linkUrl }}】</template
            >
          </el-table-column>
              ...
              <ul class="list-item-actions">
                <li>
                  <el-button
                    ...
                    @click="handleDelete(scope.row.linkId)"
                    >删除</el-button
                  >
                  <el-button
                    type="success"
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$http
        .post(
          // `/syssystem/tb-link/${this.currentPage}/${this.pageSize}`,
          "/syssystem/tb-link?currentPage=" +
            this.currentPage +
            "&pageSize=" +
            this.pageSize,
          qs.stringify(this.searchformData)
        )
        .then(function (resp) {
    ...
    changeStatus(linkId, isDisable) {
      var that = this;
      this.$http
        .post(`/syssystem/tb-link/updisable/${linkId}/${isDisable}`)
        .then(function (resp) {
          if (resp.data.code === 2000) {
            that.$message({
              message: resp.data.msg,
    ...
    handleDelete(linkId) {
      var that = this;
      this.$http.delete(`/syssystem/tb-link/${linkId}`).then(function (resp) {
        if (resp.data.code === 2000) {
          that.$message({
            message: resp.data.msg,
            type: "success",
