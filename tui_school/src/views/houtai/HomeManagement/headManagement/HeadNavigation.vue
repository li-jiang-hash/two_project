...
<template>
  <div class="pad20">
    <el-card class="box-card">
      ...
      <!--表格数据-->
      <el-table :data="tableData" stripe size="medium" style="width: 100%" :border="true">
        <el-table-column prop="headId" label="序号"> </el-table-column>
        <el-table-column prop="headName" label="导航标题" align="center">
        </el-table-column>
        <el-table-column prop="href" label="接口路径" align="center"> </el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              @click="handleDelete(scope.row.headId)"
              >删除</el-button
            >
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        headId: [{ required: true, message: "请选择头部导航栏名称", trigger: "change" }],
        skipWay: [{ required: true, message: "请选择打开方式", trigger: "change" }],
      },
      editrules: {
        headName: [
  ...
  methods: {
    ...
    init() {
      this.$http
        .post(
          "/syssystem/tb-head-nav?currentPage=" +
            this.currentPage +
            "&pageSize=" +
            this.pageSize,
    ...
    addHeadNav() {
      this.$refs.ruleForms.validate((valid) => {
        if (valid) {
          this.$http
            .post("/syssystem/tb-head-nav/insertNavigation", qs.stringify(this.ruleForm))
            .then((resp) => {
              if (resp.data.code === 2000) {
                this.$message({
                  message: resp.data.msg,
    ...
    editHeadNav() {
      this.$refs.editruleForms.validate((valid) => {
        if (valid) {
          this.$http
            .post(
              "/syssystem/tb-head-nav/insertNavigation",
