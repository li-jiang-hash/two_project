    ...
    <el-card class="box-card">
      ...
      <div>
        ...
        >
          ...
          <el-table-column label="状态" prop="isDisable" align="center" width="200">
            <template slot-scope="scope">
              <el-switch
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$http
        .post(
          //`/syssystem/tb-banner/${this.currentPage}/${this.pageSize}`,
          "/syssystem/tb-banner?currentPage=" +
            this.currentPage +
            "&pageSize=" +
            this.pageSize,
    ...
    changeStatus(id, isDisable) {
      var that = this;
      this.$http
        .post(`/syssystem/tb-banner/updisable/${id}/${isDisable}`)
        .then(function (resp) {
          if (resp.data.code === 2000) {
            that.$message({
              message: resp.data.msg,
    ...
    handleDelete(id) {
      var that = this;
      this.$http.delete(`/syssystem/tb-banner/${id}`).then(function (resp) {
        if (resp.data.code === 2000) {
          that.$message({
            message: resp.data.msg,
            type: "success",
